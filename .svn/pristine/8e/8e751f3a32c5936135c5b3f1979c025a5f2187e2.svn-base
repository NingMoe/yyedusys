...
namespace YY.Edu.Sys.Api.Services
{
    public class TeachingScheduleService
    {
        ...
        public static bool IsBeBook(int pKID, int venueID)
        {
            ...
            var isStudentBuySql = "SELECT COUNT(CurriculumID)  FROM [SportsDB].[dbo].[Curriculum]  where PKID=@pkId and [State]=@state";
            ...
            string remark)
        {
            ...
            try
            {
                ...
                var sql = "SELECT StudentID  FROM [SportsDB].[dbo].[Curriculum]  where PKID=@pkID and [State]=@state";
