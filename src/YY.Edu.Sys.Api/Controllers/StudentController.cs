...
where v.VenueID = @VenueID and s.state=1 ";
        ...
        public IHttpActionResult GetStudentCurriculum(string query)
        {
            ...
            try
            {
                ...
                criteria.PageSize = oData.PageSize;
                criteria.TableName = "TeachingSchedule t with(nolock) inner join Venue v with(nolock) on t.VenueID = v.VenueID left join Campus c with(nolock) on t.CampusID = c.CampusID inner join Curriculum cu with(nolock) on t.PKID = cu.PKID inner join Coach co with(nolock) on co.CoachID=t.CoachID ";
        ...
        public IHttpActionResult GetTeachingScheduleByStudent(string query)
        {
            try
            {
                ...
                //过滤掉超过上课时间的课程
                criteria.Condition += string.Format(" and (CurriculumDate>'{0}' or (CurriculumDate>='{0}' and CurriculumBeginTime>'{1}')) ",
                    DateTime.Now.ToString("yyyy-MM-dd"), DateTime.Now.ToString("HH:mm"));

                criteria.Condition += " and cm.CurriculumID is null and(t.State = 0 or(t.State = 1 and t.PKType = 2)) ";
                ...
                criteria.Sort = "CurriculumDate ,CurriculumBeginTime,t.CoachID, t.pkid";
        ...
        //约课
        [HttpPost]
        public IHttpActionResult SubscribeCurriculum(dynamic bookClass)
        {

            int sid = Convert.ToInt32(bookClass.SID);
            int pkid = Convert.ToInt32(bookClass.PKID);
            int cid = Convert.ToInt32(bookClass.CID);
            int vid = Convert.ToInt32(bookClass.VenueID);
            string sname = bookClass.SName.ToString();
            ...
            try
            {
                ...
                {
                    return Ok(Comm.ResponseModel.ResponseModelBase.GetRes("约课失败"));
                    // return Content(HttpStatusCode.OK, Comm.ResponseModel.ResponseModelBase.GetRes(Comm.ResponseModel.ResponseModelErrorEnum.SystemError));
                }
